...
def polygons_copy_(me, dict_1):
    list_f = [ f.index for f in me.polygons if f.select ]
    for fi in list_f:
        list_v = list(me.polygons[fi].vertices[:])
        len_ = len(list_v)
        tmp = []
        for vi in list_v:
        ...
        me.polygons.add(1)
        if len_ == 3:
            me.polygons[-1].vertices = tmp
        elif len_ == 4:
            me.polygons[-1].vertices_raw = tmp
    me.update(calc_edges = True)

def polygons_copy_1(me, dict_1, n):
    list_f = [ f.index for f in me.polygons if f.select ]
...
class ref_op5(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if len(list_0) == 0:
                self.report({'INFO'}, 'No polygons stored in memory unable to continue.')
                edit_mode_in()
                return {'CANCELLED'}
        elif len(list_1) == 0:
