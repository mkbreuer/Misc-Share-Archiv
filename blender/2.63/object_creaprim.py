...
class CreaPrim(bpy.types.Operator):
	...
	def invoke(self, context, event):
		...
		if len(objlist) > 1:
			...
			addondir = bpy.utils.script_paths()[1] + os.sep + "addons" + os.sep + groupname + os.sep
...
class MessageOperator(bpy.types.Operator):
	...
	def draw(self, context):
		...
		row.label("Add Mesh addon " + groupname + " saved to user scripts directory.")
